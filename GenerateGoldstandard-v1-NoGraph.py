...
class GenerateGoldStandard(tk.Frame):
    ...
    # def _reorder_lists_and_combos(self):
    #     self._reorder_list(self.lst_entities)
    #     self._reorder_list(self.lst_excluded_annotations)
    #
    #     self._reorder_combo(self.cmb_re_source)
    #     self._reorder_combo(self.cmb_re_target)
    ...
    def __create_GUI(self):
        ...
        tk.Button(frm_entities,  # self.lateral_frame,
                  ...
                  command=self.DeleteAnnotation).pack(side='top', fill='x', expand=True)
        ...
        # frm_rels = tk.Frame(frm_relations)  # self.lateral_frame)
        # frm_rels.pack(side='top')
        self.lst_relations = tk.Listbox(frm_relations,
                                        selectmode='single',
                                        exportselection=False,
                                        width=75)
        self.lst_relations.pack()
        ...
        #           command=self.ValidateAnnotationRelationChange).pack(side='top', fill='x', expand=True)
    ...
    def DeleteRelationAnnotation(self):
        self.lst_relations.delete(self.lst_relations.curselection())
        try:
            self.lst_relations.select_clear(self.lst_relations.curselection())
        except:
            pass
        self.selection_lst_relations_annotations = 0
        self.lst_relations.selection_set(self.selection_lst_relations_annotations)
        # self.selection_lst_relations_annotations = self.lst_relations.curselection()[0]
        self.HighlightRelation()

    # def _load_cmb_re_source_list(self):
    #  substituted by self.__fill_cmb_re_source()
    ...
    def CreateNewRelationAnnotation(self):
        ...
        # self.cmb_re_source['values'] = [f"{n_item}| {item[0]}| {item[1][SOURCE_ENTITY]}"
        #                                 for n_item, item in enumerate(self.document_entities)
        #                                 if item[0] in BEHAVIORAL_ELEMENTS]
    ...
    def DeleteEntityInRelation(self, entity_index):
        #  get deteled entity
        # print(entity_index)
        deleted_entity = self.lst_entities.get(entity_index)

        for index_rel, item in enumerate(self.lst_relations.get(0, 'end')):
            try:
                source = tuple(item[:4])
                rel_type = item[4]
                target = tuple(item[5:])
            except IndexError:
                # 3 elements instead of 9
                source, rel_type, target = item

                source = source.split(' ')
                source_n_sent, source_index_begin, source_index_end = [int(x) for x in source[-3:]]
                source_pe_type = ' '.join([x for x in source[:-3]]).strip()
                # relation_type, \
                target = target.split(' ')
                target_n_sent, target_index_begin, target_index_end = [int(x) for x in target[-3:]]
                target_pe_type = ' '.join([x for x in target[:-3]]).strip()

                source = tuple([source_pe_type, source_n_sent, source_index_begin, source_index_end])
                target = tuple([target_pe_type, target_n_sent, target_index_begin, target_index_end])

            except:
                print(len(item), item)
                assert False

            if deleted_entity == source or \
                    deleted_entity == target:
                #  remove the relation
                # self.RecordDeletedRelation(item)
                self.lst_excluded_relations.insert('end', self.lst_relations.get(index_rel))
                self.lst_relations.delete(index_rel)

    def DeleteAnnotation(self):
        self.document_is_changed.set(True)
        # self.RecordDeletedEntity(self.lst_entities.get(self.lst_entities.curselection()))
        self.DeleteEntityInRelation(self.lst_entities.curselection())
        self.lst_excluded_annotations.insert('end', self.lst_entities.get(self.lst_entities.curselection()))
        self.lst_entities.delete(self.lst_entities.curselection())
        try:
            self.lst_entities.select_clear(self.lst_entities.curselection())
        except:
            pass
        self.__draw_document_entities()
    ...
    def __draw_document_relations(self):
        for n_item in range(len(self.lst_relations.get(0, 'end'))):
            # get relation data
            ...
            self.cmb_re_source.set(' '.join([source_pe_type,
                                             str(source_n_sent),
                                             str(source_index_begin),
                                             str(source_index_end)]).split())

            self.cmb_re_type.set(rel_type)
            self.cmb_re_target.set(' '.join([target_pe_type,
                                             str(target_n_sent),
                                             str(target_index_begin),
                                             str(target_index_end)]).split())
    ...
    def HighlightPe(self, element):
        ...
        self.canvas.itemconfig(ind, {'width': self._box_border_size.get() *2})
    ...
    def LoadJsonData(self):
        ...
        self.lst_documents.delete(0, 'end')
        for doc_name in self.pet_relations.keys():
    ...
    # def _reorder_list(self, listbox_widget):
    #     elements = listbox_widget.get(0, 'end')
    #     listbox_widget.delete(0, 'end')
    #     for val in sorted(elements,
    #                       key=lambda x: (x[1], x[2]),
    #                       reverse=False):  # order by n sent and begin
    #         listbox_widget.insert('end', val)

    # def _reorder_combo(self, combobox_widget):
    #     combobox_widget['values'] = sorted(combobox_widget['values'], key=lambda x: x[0], reverse=False)
