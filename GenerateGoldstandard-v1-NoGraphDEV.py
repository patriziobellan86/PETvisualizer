...
from ShowProcessGraph import ShowGraphGUI
...
class GenerateGoldStandard(tk.Frame):
    ...
    def __create_GUI(self):
        ...
        tk.Button(frmlbl_doc_op,
                  text='Show Process Graph for Experiments',
                  command=self._show_process_graph_for_experiments).pack(side='top', fill='x')
    ...
    def _show_process_graph_for_experiments(self):
        """
        show only activity and actor performers.
        actors with the same labels are collassed in a single node
        :return:
        """
        actors = list()
        performs_relations = list()
        for rel, rel_type in deepcopy(self.document_relations):
            if rel_type == ACTOR_PERFORMER:
                performs_relations.append((rel, rel_type))
                rel.pop(SOURCE_ENTITY)
                rel.pop(SOURCE_ENTITY_TYPE)
                rel.pop(SOURCE_SENTENCE_ID)
                rel.pop(SOURCE_HEAD_TOKEN_ID)
                actor = rel
                # actors.add(actor)
        ...
        graph_to_plot = CreateGraph(_relations, self.current_doc_name)
